...
class Siswa extends CI_Controller{
	...
	function index(){
		...
		$x['wali']=$this->m_siswa->get_all_wali();
	...
	function simpan_siswa(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$sandi=$this->input->post('xsandi');
							$kelass=strip_tags($this->input->post('xwkelas'));

							$this->m_siswa->simpan_siswa($nis,$sandi,$nama,$jenkel,$kelas,$kelass,$photo);
					...
					$kelas=strip_tags($this->input->post('xkelas'));
					$sandi=$this->input->post('xsandi');
					$kela1=strip_tags($this->input->post('xwkelas'));

					$this->m_siswa->simpan_siswa_tanpa_img($nis,$sandi,$nama,$jenkel,$kelas,$kela1);
					echo $this->session->set_flashdata('msg','success');
					redirect('admin/siswa');
				}
	...
	function update_siswa(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$this->m_siswa->update_siswa($kode,$nis,$pass,$nama,$jenkel,$kelas,$photo);
	                    ...
	                    redirect('admin/siswa');
							...
							$this->m_siswa->update_siswa_tanpa_img($kode,$nis,$pass,$nama,$jenkel,$kelas);
							echo $this->session->set_flashdata('msg','info');
							redirect('admin/siswa');
	            } 
